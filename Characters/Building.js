class Building extends Gobj{
    ...
    //Life status
    lifeStatus(){
        let lifeRatio=this.life/this.get('HP');
        return ((lifeRatio>0.7)?"green":(lifeRatio>0.3)?"yellow":"red");
    };
    static ourBuildings(){
        return Building.allBuildings.filter(chara=>(!chara.isEnemy()));
    };
...
//Apply protoProps
_$.classPatch(Building);

//Zerg buildings
Building.ZergBuilding=class ZergBuilding extends Building{
    constructor(props){
        super(props);
        this.sound={
...
//Terran buildings
Building.TerranBuilding=class TerranBuilding extends Building{
    constructor(props){
        super(props);
        this.sound={
...
//Protoss buildings
Building.ProtossBuilding=class ProtossBuilding extends Building{
    constructor(props){
        super(props);
        this.sound={
...
//Apply protoProps
_$.classPatch(Building.ZergBuilding);
_$.classPatch(Building.TerranBuilding);
_$.classPatch(Building.ProtossBuilding);
...
Building.Attackable={
    ...
    prototypePlus:{
                        ...
                        if (myself.isReloaded && myself.isReloaded()) {
                            ...
                            if (myself.AOE) {
                                ...
                                switch(myself.attackLimit){
                                    case "flying":
                                        enemies=Unit.allUnits.filter(chara=>{
                                    ...
                                    case "ground":
                                        ...
                                        let enemyBuildings=Building.allBuildings.filter(chara=>(chara.team!=myself.team));
                                        enemies=enemyUnits.concat(enemyBuildings);
                                        break;
                                    default:
                                        enemies=[...Unit.allUnits,...Building.allBuildings].filter(chara=>(chara.team!=myself.team));
                                        break;
                                }
                                //Range filter
                                switch (myself.AOE.type) {
                                    case "LINE":
                                        ...
                                        let N=Math.ceil(myself.distanceFrom(enemy)/(myself.AOE.radius));
                                        enemies=enemies.filter(chara=>{
                                            for (let n=1;n<=N;n++){
                                                let X=myself.posX()+n*(enemy.posX()-myself.posX())/N;
                                                let Y=myself.posY()+n*(enemy.posY()-myself.posY())/N;
                                ...
                                let fireBullet=function(){
                                    ...
                                    if (myself.continuousAttack) {
                                        ...
                                        if (myself.AOE && myself.AOE.type=="MULTIPLE"){
                                            for (let N=0;N<Math.min(myself.AOE.count,enemies.length);N++){
                                                new myself.Bullet({
                                                    from:myself,
                                                    to:enemies[N]
                                                }).fire();
        ...
        findNearbyTargets:function(){
            //Initial
            const myself=this;
            let units=Unit.allUnits.filter(chara=>(chara.team!=myself.team));
            let buildings=Building.allBuildings.filter(chara=>(chara.team!=myself.team));
            let [results,myX,myY]=[[],myself.posX(),myself.posY()];
            [units,buildings].forEach(function(charas){
                charas=charas.filter(function(chara){
                    ...
                    let [X1,Y1,X2,Y2]=[chara1.posX(),chara1.posY(),chara2.posX(),chara1.posY()];
        ...
        AI:function(){
            ...
            if (this.isAttacking()) {
                ...
                let enemy=this.highestPriorityTarget();
        ...
        die:function(){
            //Old behavior
            Building.prototype.die.bind(this)();//super.die
...
Building.ZergBuilding.Hatchery=class Hatchery extends Building.ZergBuilding{
    constructor(props){
        super(props);
        this.larvas=[];
    };
    buildZergBuilding(){
        let target=Building.ZergBuilding[this.buildName];
        let mutation=this.evolveTo({
...
Building.ZergBuilding.Lair=class Lair extends Building.ZergBuilding{
    constructor(props){
        super(props);
        this.larvas=[];
    };
...
Building.ZergBuilding.Hive=class Hive extends Building.ZergBuilding{
    constructor(props){
        super(props);
        this.larvas=[];
    };
...
Building.ZergBuilding.SporeColony=class SporeColony extends Building.ZergBuilding{
    constructor(props){
        super(props);
        ...
        return {
            ...
            items:{
                ...
                '7':{name:'ComstatStation',condition:function(){
                    return (!Game.selectedUnit.attachment || Game.selectedUnit.attachment.status=='dead') && Building.allBuildings.some(chara=>{
...
Building.TerranBuilding.MissileTurret=class MissileTurret extends Building.TerranBuilding{
    constructor(props){
        super(props);
        this.imgPos.attack=this.imgPos.dock;
        this.frame.attack=this.frame.dock;
...
Building.ProtossBuilding.PhotonCannon=class PhotonCannon extends Building.ProtossBuilding{
    constructor(props){
        super(props);
        this.imgPos.attack=this.imgPos.dock;
...
//Evolve related
Building.ZergBuilding.Egg=class Egg extends Building.ZergBuilding{
    constructor(props){
        super(props);
        this.sound={
...
Building.ZergBuilding.Cocoon=class Cocoon extends Building.ZergBuilding{
    constructor(props){
        super(props);
        this.sound={
...
Building.ZergBuilding.MutationS=class MutationS extends Building.ZergBuilding{
    constructor(props){
        super(props);
        //Hidden frames
        this.action=7;
...
Building.TerranBuilding.ConstructionS=class ConstructionS extends Building.TerranBuilding{
    constructor(props){
        super(props);
...
Building.TerranBuilding.ConstructionM=class ConstructionM extends Building.TerranBuilding{
    constructor(props){
        super(props);
        this.imgPos.dock=this.imgPos.step1;
    };
...
Building.TerranBuilding.ConstructionL=class ConstructionL extends Building.TerranBuilding{
    constructor(props){
        super(props);
        this.imgPos.dock=this.imgPos.step1;
    };
...
Building.TerranBuilding.ConstructionR=class ConstructionR extends Building.TerranBuilding{
    constructor(props){
        super(props);
        this.imgPos.dock=this.imgPos.step1;
    };
...
Building.TerranBuilding.ConstructionF=class ConstructionF extends Building.TerranBuilding{
    constructor(props){
        super(props);
...
Building.ProtossBuilding.ArchonEvolve=class ArchonEvolve extends Building.ProtossBuilding{
    constructor(props){
        super(props);
        //Hidden frames
        this.action=7;
...
Building.ProtossBuilding.DarkArchonEvolve=class DarkArchonEvolve extends Building.ProtossBuilding{
    constructor(props){
        super(props);
        //Hidden frames
        this.action=7;
...
Building.ProtossBuilding.WrapRift=class WrapRift extends Building.ProtossBuilding{
    constructor(props){
        super(props);
        //Hidden frames
        this.action=7;
...
_$.classPackagePatch(Building.ZergBuilding);
_$.classPackagePatch(Building.TerranBuilding);
_$.classPackagePatch(Building.ProtossBuilding);
