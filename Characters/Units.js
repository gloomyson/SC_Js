...
class Unit extends Gobj{
    ...
    animeFrame(){
        ...
        let arrLimit=(this.imgPos[this.status].left[0] instanceof Array)?(this.imgPos[this.status].left[0].length):1;
    ...
    detectOutOfBound(){
        let [boundX,boundY]=[(Map.getCurrentMap().width-this.width),
            (Map.getCurrentMap().height-this.height)];
        //Right Bound
        if (this.x>boundX) {
            this.x=boundX;
        }
    ...
    navigateTo(clickX,clickY,range){
        ...
        //Center position
        let [charaX,charaY]=[this.posX(),this.posY()];
    ...
    moveToward(target,range,callback=function(){}){
        if (!range) range=Unit.moveRange;//Smallest limit by default
        //Start new routing
        const myself=this;
        let routingFrame=function(){
    ...
    //####Class.methods####//
    static allOurUnits(){
        return Unit.allUnits.filter(chara=>(!chara.isEnemy()));
    };
    static allEnemyUnits(){
        return Unit.allUnits.filter(chara=>(chara.isEnemy()));
    };
    static allFlyingUnits(){
        return Unit.allUnits.filter(chara=>chara.isFlying);
    };
    ...
    static getSpeedMatrixBy(speed){
        let speedMatrix=_$.clone(Unit.speedMatrix);
        _$.matrixOperation(speedMatrix,N=>(N*speed));
        return speedMatrix;
    };
    //Get all units count
    static count(){
...
class AttackableUnit extends Unit{
                        ...
                        if (myself.AOE) {
                            ...
                            switch(myself.attackLimit){
                                ...
                                case "ground":
                                    ...
                                    enemies=[...enemyUnits,...enemyBuildings];
                                    break;
                                default:
    ...
    findNearbyTargets(){
        ...
        let results=[];
        let targets=[
            units.filter(chara=>chara.attack),
            buildings.filter(chara=>chara.attack),
            units.filter(chara=>(!chara.attack)),
            buildings.filter(chara=>(!chara.attack))
        ];
        targets.forEach(function(charas){
            let [myX,myY]=[myself.posX(),myself.posY()];
            charas=charas.filter(chara=>{
                ...
                let [X1,Y1,X2,Y2]=[chara1.posX(),chara1.posY(),chara2.posX(),chara2.posY()];
